...
- [docs/12_从标定板参数确认到完成手眼标定教程.md](docs/12_从标定板参数确认到完成手眼标定教程.md)：按实操顺序串起标定板参数确认、采集、计算和结果检查。
- [docs/13_标定板映射与手眼结果正确性检查.md](docs/13_标定板映射与手眼结果正确性检查.md)：检查标定板映射文件、映射可视化和手眼标定输出是否可信。
- [docs/14_rot与标定板姿态关系.md](docs/14_rot与标定板姿态关系.md)：解释 `rot` 如何影响 PnP/手眼标定，以及建映射姿态和采集姿态的关系。
- [docs/15_法兰盘中心与夹爪TCP对标定的影响.md](docs/15_法兰盘中心与夹爪TCP对标定的影响.md)：说明 RTDE 读法兰盘中心或夹爪 TCP 时，对手眼标定、固定相机外参和抓取目标的影响。
- [docs/16_GraspNet输出到夹爪TCP目标点位推理.md](docs/16_GraspNet输出到夹爪TCP目标点位推理.md)：基于夹爪 TCP 眼在手上结果，推导 GraspNet 输出到 UR TCP 抓取目标的矩阵链路。
