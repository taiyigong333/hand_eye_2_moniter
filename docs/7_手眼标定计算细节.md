...
| --- | --- |
| `base` | 机器人基座坐标系 |
| `ee` / `gripper` | 机器人末端参考系，代码中把 RTDE 保存的法兰盘中心或夹爪 TCP 位姿当作 `T_base_ee`；两者都在 `base` 坐标系下表达，区别见 [15_法兰盘中心与夹爪TCP对标定的影响.md](15_法兰盘中心与夹爪TCP对标定的影响.md) |
| `cam_end` | 安装在末端的腕部相机 |
...
第一段，读取数据。`load_samples()` 扫描 `dataset_dir` 下的 `*.json`，只保留同时包含 `tcp_pose` 和 `images` 的样本。`eye_in_hand` 模式只根据 `--end_camera_index` 查找末端相机图像；`dual_camera` 模式还会根据 `--fixed_camera_index` 查找固定相机图像。`tcp_pose_to_T_base_ee()` 把 `[x, y, z, rx, ry, rz]` 转成 `T_base_ee`，其中 `rx, ry, rz` 是 Rodrigues 旋转向量。这里的 `tcp_pose` 可以是机器人 `base` 下的法兰盘中心位姿，也可以是机器人 `base` 下的夹爪 TCP 位姿，但整批样本和下游解释必须保持一致。
