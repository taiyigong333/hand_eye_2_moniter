...
- `calibration`：采集结束后传给 `calib.py` 的参数，其中 `calibration.mode` 控制 `dual_camera` 或 `eye_in_hand`。
...
采集默认会打开两个预览窗口。手动模式默认按 `c` 保存一次，按 `q` 结束；预览窗口聚焦时也能直接使用这两个按键。无桌面显示时可加 `--no_preview`。更多细节见 `docs/4_实时采集与自动标定流程.md`。
