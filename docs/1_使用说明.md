...
本项目用于基于 ArUco 标定板、UR7e TCP 位姿和 RealSense 图像，完成以下外参估计：

- `T_ee_cam_end`：末端腕部相机到机械臂末端坐标系的变换，属于眼在手上结果。
- `T_base_board`：标定板到机器人基座坐标系的变换。
- `T_base_cam_fixed`：外部固定相机到机器人基座坐标系的变换，属于双相机联合流程下的固定相机外参。
...
- `scripts/run_current_calibration.ps1`：使用本地样例数据运行双相机联合标定。
- `scripts/run_eye_in_hand_calibration.ps1`：使用本地样例数据只运行眼在手上标定。
...
`calib.py` 期望 `--dataset_dir` 下递归存在若干 `pose.json`。`--mode eye_in_hand` 只要求每个有效样本包含末端相机图像；`--mode dual_camera` 还要求固定相机图像。当前样本格式如下：
...
- `camera_index=1` 当前代表固定相机，双相机联合模式中传给 `--fixed_camera_index`。
- `eye_in_hand` 模式要求末端相机能看到标定板；`dual_camera` 模式要求两台相机都能看到标定板。当前模式所需相机检测到的 marker 数量不能少于 `--min_tags`。
...
本地 `data/dataset_from_npz/` 数据对应的双相机联合标定命令：
...
- `dynamic_end_camera_poses.json`：每个有效样本下动态末端相机在基座坐标系中的位姿；`dual_camera` 模式还包含末端相机相对固定相机的位姿。
- `debug_vis/*.jpg`：每张图像的检测角点和 PnP 重投影调试图。

`calibration_result.json` 中主要字段：

- `num_total_samples`：读取到的样本数。
- `num_valid_samples`：通过当前模式所需相机检测和 RMSE 过滤后的样本数。
- `mode`：`dual_camera` 或 `eye_in_hand`。
- `T_ee_cam_end`：末端相机到末端执行器坐标系的变换。
- `T_base_board`：标定板到机器人基座坐标系的变换。
- `T_base_cam_fixed`：固定相机到机器人基座坐标系的变换，仅 `dual_camera` 模式输出。
...
- `T_base_board` 的一致性统计是否没有明显恶化；`dual_camera` 模式还要检查 `T_base_cam_fixed` 的一致性统计。
...
- `cameras`：RealSense 的序列号、分辨率、图像文件名、内参保存路径和可选深度图开关；`eye_in_hand` 只启动末端相机。
- `preview`：采集时的 OpenCV 预览窗口，默认开启。
- `sampling`：`timed` 定时采集或 `manual` 手动采集。
- `calibration`：采集结束后传给 `calib.py` 的参数，其中 `calibration.mode` 控制 `dual_camera` 或 `eye_in_hand`；命令行可用 `--calibration_mode eye_in_hand` 临时覆盖。
...
采集默认会打开当前标定模式所需相机的预览窗口；`eye_in_hand` 只启动末端相机。手动模式默认按 `c` 保存一次，按 `q` 结束；预览窗口聚焦时也能直接使用这两个按键。无桌面显示时可加 `--no_preview`。更多细节见 `docs/4_实时采集与自动标定流程.md`。
